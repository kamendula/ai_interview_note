## 背景
对于很多刚开始刷题的同学来说，面对 LeetCode 上近两千道题，确实容易陷入迷茫，不知道从何刷起。很多人感到刷题效率低，时间浪费主要集中在三点：

* 找题耗时：难以在海量题目中挑选出适合自己当前水平的题目。
* 题目难度不匹配：做了不适合现阶段能力的题目，导致进展缓慢甚至打击自信心。
* 缺乏优质题解：刷完题后找不到全面清晰的题解，无法有效提高解题思路。

所以我建议按照数据结构和算法的类型顺序来刷题，比如从数组开始，依次刷链表、哈希表、字符串、栈与队列、树、回溯、贪心、动态规划、图论、以及高级数据结构等。按难度从简单题入手，慢慢过渡到中等题和困难题。这个顺序有助于系统化学习和巩固知识。

然而，即便有这样的整体规划，许多初学者甚至有经验的算法老手依然会在寻找合适的题目时耗费大量时间，并且不一定找到经典的题目。因此，为了提高效率，最理想的做法就是用最短的时间，按循序渐进的难度刷完所有经典题目。基于此，我整理了 LeetCode 刷题攻略，提供一个详细的刷题顺序。每道题都是经过精挑细选的经典高频面试题，只需按照这个顺序刷题即可，完全不需要再在题海中迷失方向。同时，我为每道题都撰写了详细的题解，图文并茂，针对难点还配有视频讲解。我在 LeetCode 上的题解通常位于题目首页，质量有口皆碑。

因此，我的刷题顺序完全为那些希望在最短时间内系统掌握算法的同学准备，不仅省时高效，还能让你在算法知识体系上取得质的飞跃。如果你正在刷 LeetCode，强烈建议按照本攻略中的刷题顺序进行，你会感受到系统学习带来的巨大收获。不用再为找题发愁，也不必在茫茫题海中寻找方向。

如何使用这份刷题攻略？
你只需要按顺序从数组开始刷题即可，攻略中所有题目已经按照合理的学习顺序排列好，完全不用再自行挑选。在每个专题开始前，我还归纳了所需的基础理论，避免像教科书那样枯燥乏味，而是从实战经验中提炼出重点知识。每个专题结束时也有总结篇，对该专题进行归纳和反思。

无论你是初学者还是算法老手，这份攻略都能帮助你复习和巩固。对于老手来说，快速过一遍每个系列的总结篇，整个算法知识体系会再次浮现脑海。
